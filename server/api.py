...
###
from server.kg_transformer_api import texts_to_graphs
...
def mount_app_routes(app: FastAPI, run_mode: str = None):
    ...
    #新添加文本转图谱接口
    app.post("/other/texts_to_graphs",
            tags=["Other"],
            summary="将文本转换成知识图谱，正在测试在线LLM模型",
            )(texts_to_graphs)
...
def mount_knowledge_routes(app: FastAPI):
    ...
    ###
    from server.kg_transformer_api import files2graphs
    app.post("/knowledge_base/files_to_graphs",
             tags=["Knowledge Base Management"],
             response_model=BaseResponse,
             summary="将文件转换成知识图谱，正在测试在线LLM模型"
             )(files2graphs)
    
    from server.kg_transformer_api import docs2graphs
    app.post("/knowledge_base/docs_to_graphs",
             tags=["Knowledge Base Management"],
             response_model=BaseResponse,
             summary="将向量转换成知识图谱，正在测试在线LLM模型"
             )(docs2graphs)
    ###
