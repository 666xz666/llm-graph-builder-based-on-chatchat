...
from configs import (LLM_MODELS, LLM_DEVICE, EMBEDDING_DEVICE,
                     ...
                     FSCHAT_MODEL_WORKERS, HTTPX_DEFAULT_TIMEOUT,KG_LLM_MODELS)
import os
...
from configs import NEO4J_URI,NEO4J_USERNAME,NEO4J_PASSWORD
def create_graph_database_connection(database='neo4j'):
        uri=NEO4J_URI
        userName=NEO4J_USERNAME
        password=NEO4J_PASSWORD
        graph = Neo4jGraph(url=uri, database=database, username=userName, password=password, sanitize=True)    
        return graph
    
    
def close_db_connection(graph): 
...
class Kimi(LLM):
    ...
    def _call(self, prompt: str, **kwargs: Any) -> str:
        ...
        try:
            ...
            client = OpenAIClient(
                #此处请替换自己的api
                api_key=ONLINE_LLM_MODEL['moonshot-v1-8k']["api_key"],
...
def get_QianfanChatEndpoint():
    from langchain_community.chat_models import QianfanChatEndpoint
    os.environ["QIANFAN_AK"] = ONLINE_LLM_MODEL['qianfan-api']['api_key']
    os.environ["QIANFAN_SK"] = ONLINE_LLM_MODEL['qianfan-api']['secret_key']
...
def get_llm(
    ...
    if model_name=='qianfan-api':
        return get_QianfanChatEndpoint()
    elif model_name=='moonshot-v1-8k':
        return get_Kimi()
    elif model_name == 'openai-api':
        return get_ChatOpenAI(model_name,0.5)
    else:
        return None
###
